...
class App(tk.Tk):
    ...
    def _header_contents(self, canvas: tk.Canvas):
        # Logo + tytuł + podtytuł
        try:
            logo1_path = os.path.join(os.path.dirname(__file__), "LOGO19x119.png")
            if PIL_OK and os.path.exists(logo1_path):
                img1 = Image.open(logo1_path).resize((238, 38))
                self._logo_img1 = ImageTk.PhotoImage(img1)
                canvas.create_image(600, 10, image=self._logo_img1, anchor="n")
        except Exception:
            pass
        canvas.create_text(600, 60, anchor="n", text=f"{EXPERT['company']} — panel mieszkańca CWU",
                           fill=COL_TEXT, font=(FONT_FAMILY, 22, "bold"))
        canvas.create_text(600, 90, anchor="n", text=f"{EXPERT['name']}  |  {EXPERT['title']}",
                           fill=COL_MUTE, font=(FONT_FAMILY, 14))
